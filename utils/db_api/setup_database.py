import asyncio
import sys
from postgresql import Database  # Import your Database class
...
async def setup_database():
    ...
    try:
        ...
        # Insert initial multilingual messages
        await db.execute(insert_data_query, execute=True)
        print("Initial multilingual messages inserted successfully!")

    except Exception as e:
        print(f"Error setting up database: {e}")

# SQL query to insert the "About Us" text in three languages
    ...
    async def insert_about_us():
        db = Database()
        await db.create()  # Connect to the database
        try:
            await db.execute(insert_about_us_query, execute=True)
            print("About Us messages inserted successfully!")
        except Exception as e:
            print(f"Error inserting 'About Us' messages: {e}")
        finally:
            print("Done.")

if __name__ == "__main__":
    # asyncio.run(setup_database())
    asyncio.run(insert_about_us())
